...
main() {
  final dio = MockClient();
  final repository = SuperHeroRepository(dio);

  final repositoryFromInternet = SuperHeroRepository(Dio());

  group('Repository from internet', () {
    test('The list of superheroes should not be null', () async {
      final list = await repositoryFromInternet.getAll();

      expect(list, isNotNull);
    });

    test('The list of superheroes should not be empty.', () async {
      final list = await repositoryFromInternet.getAll();
